...
UPDATE "USER" SET ROLE = 'COACH' WHERE Email = 'lee2000@hexschooltest.io' AND ROLE = 'USER';
UPDATE "USER" SET ROLE = 'COACH' WHERE Email = 'muscle@hexschooltest.io' AND ROLE = 'USER';
UPDATE "USER" SET ROLE = 'COACH' WHERE Email = 'starplatinum@hexschooltest.io' AND ROLE = 'USER';
...
UPDATE "COURSE_BOOKING" SET cancelled_at='2024-11-24 17:00:00',status='已取消' 
...
SELECT "COURSE_BOOKING".user_id,("COURSE_BOOKING".total_credit - "CREDIT_PURCHASE".total_credit) remaining_credit
FROM 
(
    SELECT user_id,SUM(purchased_credits) total_credit
    FROM "CREDIT_PURCHASE" 
    WHERE user_id = (SELECT id FROM "USER" WHERE email='wXlTq@hexschooltest.io')
    GROUP BY user_id
) as "COURSE_BOOKING" 
INNER JOIN
(
    SELECT user_id,COUNT(*) total_credit
    FROM "COURSE_BOOKING" 
    WHERE user_id = (SELECT id FROM "USER" WHERE email='wXlTq@hexschooltest.io') 
    AND status <> '已取消'
    GROUP BY user_id
...
SELECT s.name "專長名稱",COUNT(*) coach_total
...
-- 要計算到12月才會有值~~~~
select cp2.name "組合包方案名稱",count(*) "銷售數量"
